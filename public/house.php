...
<div class="w-full h-screen">
  ...
  <div class="fixed bottom-0 w-full h-16 bg-white shadow-inner">
    <div class="max-w-screen-lg mx-auto h-full flex justify-end items-center">
      ...
      <a href="./collect.php?house_id=<?=$house['id']?>"><button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 rounded-lg text-lg px-5 py-2.5 text-center ml-2 dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:hover:bg-blue-600 dark:focus:ring-blue-800"><?=$collected?'已收藏':'加入收藏'?></button></a>
      <?php endif; ?>
      <?php if (isset($_SESSION['user_type']) && $_SESSION['user_type'] == 'tenant'): ?>
        <a href="./chatroom.php?to_message=<?=$house['landlord_id']?>">
      <?php endif; ?>
